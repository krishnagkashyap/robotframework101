...
#########################################################################################################################
   ...
   Set To Dictionary       ${3ldict}     3l.tokenidtoken.resource4_url=&{Init}[oam.3l.ohshost.ohsport]/oauth2/rest/authorize?response_type=id_token token&domain=&{iddomaindetails}[name]&client_id=&{clientdetails}[name]&scope=RServer0.playsong&{test_data}[3l.scope.seperator]RServer0.searchsong&{test_data}[3l.scope.seperator]openid&{test_data}[3l.scope.seperator]profile&state=code1234&redirect_uri=&{3ldict}[oam.3l.vars.redirectURI0]&nonce=testidtoken
    ...
    Set To Dictionary       ${idparameters}     identityProvider=&{Init}[oam.3l.identityProvider]
...
Set Common Test Variables
    ...
    Set To Dictionary       ${clientdetails}     clientname=&{test_data}[3l.client.secret]
    Set To Dictionary       ${clientdetails}     secret=&{test_data}[3l.client.secret]
...
3L_AuthzCode_CL0_Request_Scope_FullList_1
    ...
    ${accesstoken}      ${refreshtoken}=     OAUTHRUNTIME AUTHORIZATION_CODE GET ACCESSTOKEN AND REFERSHTOKEN      &{iddomaindetails}[name]     &{clientdetails}[id]     &{clientdetails}[secret]     ${authzcode}      &{3ldict}[oam.3l.vars.redirectURI0]

    ${response}=        ACCESS TOKEN VALIDATION FLOW    &{iddomaindetails}[name]        ${accesstoken}
...
#    LOG TO CONSOLE      &{validatedresponse}[isClaimsValidFlag]
    ...
    ${response_userinfo}=        ACCESS TOKEN VALIDATION USERINFO FLOW    &{iddomaindetails}[name]       ${accesstoken}
...
3L_AuthzCode_CL0_Request_Scope_FullList_1_RT
    ...
    ${accesstoken_from_refreshtoken}=       FETCH ACCESSTOKEN FROM REFRESH TOKEN FLOW    &{iddomaindetails}[name]      &{clientdetails}[id]     &{clientdetails}[secret]    RServer0.playsong   ${refreshtoken}

    ${response}=        ACCESS TOKEN VALIDATION FLOW    &{iddomaindetails}[name]       ${accesstoken_from_refreshtoken}
    Run Keyword And Ignore Error    Should Contain      ${response}     &{iddomaindetails}[name]    ignore_case=True
    Run Keyword And Ignore Error    Should Contain      ${response}     &{clientdetails}[name]      ignore_case=True
    Run Keyword And Ignore Error    Should Contain      ${response}     &{Init}[oam.admin.username]     ignore_case=True
#
...
3L_AuthzCode_CL0_Request_Scope_FullList_2_RT
    ...
    ${accesstoken_from_refreshtoken}=       FETCH ACCESSTOKEN FROM REFRESH TOKEN FLOW    &{iddomaindetails}[name]      &{clientdetails}[id]     &{clientdetails}[secret]    RServer0.searchsong   ${refreshtoken}
...
#    ${response_userinfo}=        ACCESS TOKEN VALIDATION USERINFO FLOW    &{iddomaindetails}[name]       ${accesstoken_from_refreshtoken}
#    &{validUserInfoclaims}      create dictionary
#    Set To Dictionary       ${validUserInfoclaims}     profile=null
#
#    &{validatedUserInfoResponse}=   VALIDATEUSERINFORESPONSE    ${response_userinfo}     ${validUserInfoclaims}
